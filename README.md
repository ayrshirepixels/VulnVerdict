...
This repository covers all five phases of the build brief: feeds, watchlist, decision table and evidence chains (1), the console, digest and workflow (2), the inventory connectors, needs-mapping queue and coverage reconciliation (3), vendor advisories, native ticket adapters, compensating controls, auto-close, the weekly report, API and webhooks (4), and the central enrichment service with signed bundles, licence tiers, air-gap mode, MSP portal, telemetry and the appliance build (5). See `docs/` for install, connectors, the digest, the API and security.
...
- **Inventory connectors** (each one credential form, read-only, never writes to the source): FortiClient EMS, Windows servers over WinRM or OpenSSH (roles, features, listeners, IIS, SQL Server, .NET, Hyper-V VM list), FortiGate (firmware, switches, APs, and exposure from VIPs, policies and WAN services), VMware vCenter, Linux over SSH (packages matched through OSV), SBOM upload or URL (CycloneDX, SPDX), SNMP for the management network, a pure .NET discovery sweep, and an external cross-check with optional Shodan. Assets are merged across sources; unmapped software goes to the Needs mapping queue; stale assets fall out of digests after 30 days and are archived after 90.
- **Vendor advisories**: Fortinet PSIRT, Microsoft MSRC, Cisco openVuln (with credentials), Broadcom/VMware, Ubuntu, Debian and Red Hat feed the evidence chain and the "exploited in the wild" signal.
...
- **Central service** (`src/VulnVerdict.Central`): builds signed feed bundles hourly, issues licence keys, hosts the MSP portal, receives opt-in telemetry, serves the pricing page. Consoles verify the ECDSA signature and every file hash and refuse unsigned or downgraded bundles; air-gapped sites upload the same bundle by hand.
