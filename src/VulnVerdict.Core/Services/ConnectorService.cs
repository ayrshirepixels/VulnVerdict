...
/// <summary>
/// Section 9.1: connectors are adapter instances with a hostname and a read-only credential. Credentials live
/// encrypted in the database (never in configuration files or logs). Three consecutive failures raise the
/// administrator alert; adapters never write to the source system.
/// </summary>
public sealed class ConnectorService
{
    ...
    public ConnectorService(IDbContextFactory<VvDbContext> factory, IEnumerable<IInventoryAdapter> adapters, InventoryService inventory, IDataProtectionProvider dp, ILogger<ConnectorService> log)
    {
        _factory = factory; _adapters = adapters; _inventory = inventory; _log = log;
    ...
    {
        var adapter = Adapter(adapterId) ?? throw new ArgumentException("Unknown adapter " + adapterId);
        await using var db = await _factory.CreateDbContextAsync(ct);
        var c = id is null ? null : await db.Connectors.FirstOrDefaultAsync(x => x.Id == id, ct);
        var now = DateTime.UtcNow;
        if (c is null) { c = new Connector { Id = Guid.NewGuid(), AdapterId = adapter.Metadata.Id, CreatedAt = now }; db.Connectors.Add(c); }
        // blank password fields keep the stored value
        var current = Decrypt(c);
        foreach (var f in adapter.Metadata.Form)
        {
        ...
        var missing = adapter.Metadata.Form.Where(f => f.Required && string.IsNullOrWhiteSpace(credentials.GetValueOrDefault(f.Key))).Select(f => f.Label).ToList();
        if (missing.Count > 0) throw new ArgumentException("Required: " + string.Join(", ", missing));
        c.DisplayName = string.IsNullOrWhiteSpace(displayName) ? adapter.Metadata.DisplayName : displayName.Trim();
        ...
        db.Audit.Add(new AuditEntry { At = now, Actor = actor, Action = id is null ? "connector.add" : "connector.update", Target = c.DisplayName, After = adapter.Metadata.Id + (enabled ? "" : " (disabled)") });
    ...
    public async Task<TestResult> TestAsync(string adapterId, Dictionary<string, string> credentials, Guid? existingId, CancellationToken ct = default)
    {
        ...
        try { return await adapter.TestAsync(credentials, ct); }
    ...
    /// <summary>Run one connector end to end. Used by the worker on schedule and by "Run now".</summary>
    public async Task<ApplySummary?> RunAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (adapter is null) { c.LastError = "Adapter " + c.AdapterId + " is not installed"; c.RunRequested = false; await db.SaveChangesAsync(ct); return null; }
